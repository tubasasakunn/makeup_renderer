...
def MakeParams(preset,make_name,right=True,renderMode='powder',useMirror=True):

    config={}
    if make_name in ["Foundation", "Lipstick"]:
        for i,key in enumerate(['RGB', 'alpha']):
            for j in range(len(preset)):
                noise=random.normalvariate(0,0.35)
